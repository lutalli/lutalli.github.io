...
{:.no-indent}
- The set of all $\kappa$-[sized subsets](/codex/SSUB) of $X$ is denoted by $\powerset_\kappa(X)$.

- The [equivalence class](/codex/EQV#DEF-EQV-CLS) of $a$ with respect to $\sim$ is denoted by $[a]_{\sim}$.

- The set of all [functions](/codex/F) from $X$ to $Y$ is usually denoted by $\fun(X,Y)$.
